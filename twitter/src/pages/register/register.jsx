...
export default function Register(){
	...
	let onSubmit = async() => {
		try {
			// Step1. Ambil inputan user
			let inputEmail = email.current.value
			let inputPassword = password.current.value 

			// Step2. Validasi

			// Step3. Simpan datanya ke dalam database
			let dataToSend = {email: inputEmail, password: inputPassword}
			let response = await axios.post('http://localhost:5000/users', dataToSend)
			email.current.value = ''
    ...
    return(
        <div className="container max-w-lg">
			<form>
				...
				<button onClick={onSubmit} type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Register</button>
			</form>
		</div>
    )
}